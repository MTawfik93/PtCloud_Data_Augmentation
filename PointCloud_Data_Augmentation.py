...
# Translate Scaled Function to the Ground
...
Dataset = r'F:\Work\Comark\Trucks augmented data\Transit' #Path to Dataset

RandomDownsample_Folder = r'F:\Work\Comark\Trucks augmented data\Transit\RandomDownsample' #Path to RDownsample Folder
UniformDownsample_Folder = r'F:\Work\Comark\Trucks augmented data\Transit\UniformDownsample' #Path to UDownsample Folder
Scale_Folder = r'F:\Work\Comark\Trucks augmented data\Transit\Scale' #Path to Scale Folder
Rotate_Folder = r'F:\Work\Comark\Trucks augmented data\Transit\Rotate' #Path to Rotate Folder
Translate_Folder = r'F:\Work\Comark\Trucks augmented data\Transit\Translate' #Path to Translate Folder
Jitter_Folder = r'F:\Work\Comark\Trucks augmented data\Transit\Jitter'
...
if os.path.isdir(RandomDownsample_Folder) and os.path.isdir(UniformDownsample_Folder) and \
    ...
    os.mkdir(RandomDownsample_Folder) # If the folders aren't there, Create them and do the same process
...
for filename in os.listdir(Dataset): # Scans the directory for files
    ...
    if filename.endswith(".pcd") or filename.endswith(".ply"): #Check for files ending with .pcd or .ply
        ...
        New_filename_RD = str("RandomDown_Transit_") + str(i) + str(".pcd") # Renames the new files
        New_filename_UD = str("UniformDown_Transit_") + str(i) + str(".pcd")
        #New_filename_S = str("Scale_Scooter_") + str(i) + str(".pcd")
        #New_filename_J = str("Jitter_Transit_") + str(i) + str(".pcd")
        New_filename_J = str("Jitter_Transit_") + str(i) + str(".pcd")

        # New_filename_RX = str("Rotate_ScooterX_") + str(i) + str(".pcd")
        # New_filename_RY = str("Rotate_ScooterY_") + str(i) + str(".pcd")
        # New_filename_RZ = str("Rotate_ScooterZ_") + str(i) + str(".pcd")

        # New_filename_TX = str("Translate_ScooterX_") + str(i) + str(".pcd")
        # New_filename_TY = str("Translate_ScooterY_") + str(i) + str(".pcd")
        # New_filename_TZ = str("Translate_ScooterZ_") + str(i) + str(".pcd")
